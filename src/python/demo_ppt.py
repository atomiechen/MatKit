...
def main(args):
    ...
    my_selector_qr = PressureSelector(levels=[args.click, 5*args.click],
                                      mode="quickrelease")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', dest='server_addr',
                        action='store', help="specify server socket address")
    parser.add_argument('-c', dest='client_addr',
                        action='store', help="specify client socket address")
    parser.add_argument('-u', '--udp', dest='udp',
                        action='store_true', default=UDP, help="use UDP protocol")
    parser.add_argument('-n', dest='n', action='store',
                        default=N, type=int, help="sensor side size")
    parser.add_argument('--mapcoor', dest='mapcoor', action='store_true',
                        default=False, help="map coordinates directly")
    parser.add_argument('--trackpoint', dest='trackpoint',
                        action='store_true', default=TRACK_POINT, help="TrackPoint style")
    parser.add_argument('--interp', dest='interp', action='store',
                        default=INTERP, type=int, help="interpolated side size")
    parser.add_argument('--th', dest='threshold', action='store',
                        default=TH, type=float, help="blob filter threshold")
    parser.add_argument('--alpha', dest='alpha', action='store',
                        default=ALPHA, type=float, help="point smoothing value")
    parser.add_argument('--click', dest='click', action='store',
                        default=CLICK, type=float, help="click value")
